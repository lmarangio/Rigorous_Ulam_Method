...
from matrix_io import save_binary_matrix
import tempfile
import subprocess
import os
import re
...
def rigorous_residual(P, v, norm='L1'):
    ...
    assert all(q >= 0 for q in v)
    
    intervalfield = P.base_ring()
    
    #I tried to use change_ring() and apply_map() instead of
        #these for loops, but they are too slow for large dimensions
...
GPU_ENV_VAR="COMPINVMEAS_OPENCL_DIR"
GPU_CONTRACTION_COMMAND="./ContractionSpM"
GPU_SUPPORTED_NORMS = {
        "L1" : "Norm_L1",
        "Linf" : "Norm_Linf"
}

def compute_decay_time_gpu(P, target, steperror, norm):
    print "Computing the decay time using the GPU!"
    if not norm in GPU_SUPPORTED_NORMS:
        raise ValueError, 'Norm '+norm+' not supported on the GPU!'
    tempdir = tempfile.mkdtemp()
    matrixname = tempdir+'/matrix.bin'
    save_binary_matrix(P, matrixname)
    args = [GPU_CONTRACTION_COMMAND,
            '-b', matrixname,
            '-a', str(target),
            '-s', '%.10f' % steperror,
            '-N', GPU_SUPPORTED_NORMS[norm],
            '-f', 'd']
    cwd = os.environ.get(GPU_ENV_VAR)
    print "Running: ",cwd, " ".join(args)
    out = subprocess.check_output(args, cwd=cwd)
    shutil.rmtree(tempdir)
    print out
    m = re.search("^CONTRACTION IN (\\d+) STEPS$", out, flags=re.MULTILINE)
    if not m:
        raise ValueError,"Something went wrong, no contraction detected!"
...
def decay_time(P, alpha = 0.5, n_jobs=1, norm='L1'):
    ...
    if norm == 'L1':
        ...
        target = Rdown(alpha) / P.dimensions()[0] / 2 #we need an additional factor
                                                      #0.5 here because the vectors that we
                                                      #use have inf-norm 0.5 rather than 1
    elif norm == 'Linf_C1':
    ...
    if os.environ.get(GPU_ENV_VAR):
        return compute_decay_time_gpu(P, target, steperror, norm)

    P = sage_sparse_to_scipy_sparse(P.apply_map(lambda x: RR(x.center())))
    n = P.shape[0]

    decayTimes = Parallel(n_jobs=n_jobs, verbose=1)(
                delayed(decay_time_helper)(P,k,target = target,
                                           steperror = steperror,
                                           norm = norm_function)
                for k in range(1,n))
